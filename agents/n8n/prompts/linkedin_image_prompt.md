...
3. **Write the headline:** a compression of that tension. Aim for 5 words; 6 is the absolute maximum. Count them before you commit (it may quote or sharpen the hook; it must not be a generic topic label like "Data Engineering").
4. **Choose the mode:**
...
- **Fuse the metaphor with the anchor.** ONE central metaphor that dramatizes the hook's tension, and the subject anchor must be physically present inside it. Exactly one of these two ways:
  - the metaphor is **made of** the anchor — the anchor's own material forms the shape the metaphor needs;
  - or the metaphor **acts on** the anchor — the force in the metaphor is visibly applied to the anchor itself.
  A metaphor that merely *rhymes* with the tension, floating free of the subject, is a failure however striking it looks. Redraw it with the anchor in it.
- **Build your own fusion.** The example fusions in the motif vocabulary show the *degree* of fusion required, not a catalogue to pick from. Take their structure — a specific tension made physical in the subject's own material — and build a new object for THIS post's hook. Lifting one and changing a word produces a feed where every post looks like the last one. Take the *structure* of the examples — a specific tension made physical in the subject's own material — and build a new object for THIS post's hook. If your concept is recognisably one of the listed examples, discard it and go again.
- **If the tension is displacement, the thing doing the displacing must be in frame.** "Your skill is now a commodity" is not shown by the obsolete thing on its own — it is shown by what stands next to it. The subject only reads as finished when its replacement is visibly present and visibly working. Draw the successor, or you have drawn an admiring portrait of the old thing.
- **In a displacement image the successor is made of the anchor too.** Both objects are the subject — its old form and its new form — so both must be built from the subject's material. Blank, generic successors undo the whole image: a press stamping out plain metal blanks beside an engraved wrench is a picture about artisanship versus factories, and the software has quietly left the frame. Give the successor the same material tell as the subject — the stamped copies carry the same config engraving, the robot's replacement part is etched with the same schema — so the tension reads as *this* craft being commoditised and not craft in general.
- **The domain signal must live on whatever carries the accent.** The engraved code or config is what makes the material read as software, so it has to sit in the lit, foregrounded part of the frame. Putting the only software tell on the dim object at the edge and leaving everything bright generic fails the domain test no matter how good the composition is: at phone width the reader sees only the bright half. Whatever holds the accent must also carry the anchor's texture.
- **One metaphor, and the headline shares it.** If the headline says "drying up" the image must show a falling level, a dry channel, a last drop. A water headline over a full, brightly flowing pipe cancels itself out, and the reader feels the contradiction before they can name it. Pick the metaphor first, then write the headline inside it, and **name in the image prompt the visible thing that carries each metaphor word** — "dried up" needs the cracked bed or the last drop actually described in the prompt, not merely implied. This applies to every load-bearing word, not just the verb: a headline saying "moat" needs something moat-like — a wall, a ditch, a defended edge — visible in frame, or it must be rewritten with a word the image can actually show.
...
- **One focal point, and name what carries the accent.** Maximum 5 visual elements, generous negative space. Normally the brightest, most saturated thing in the frame IS the focal object; if unrelated machinery glows harder than the subject, the eye goes there first and the idea is lost. The one deliberate exception is a displacement image, where the accent belongs on the **successor** precisely so the subject reads as dim, dead and finished beside it. That exception is about **light only**. It is not permission to demote the subject: the subject still holds the centre, the larger share of the frame and the most rendered detail, and the successor is the smaller, brighter thing beside or behind it. A frame where the successor is bigger, higher and spotlit and the subject is a small dim prop at the edge has changed what the image is about — it is now a portrait of the machine, and the reader never looks at the thing the post is actually about. Either way, the image prompt must **say in words** which object is centre-frame, which object is physically larger, and which object carries the accent — and everything else stays dim. Naming the accent but leaving position and relative size unstated is how the successor drifts into the foreground and takes over the picture.
...
- Text other than the headline has exactly two permitted uses: (a) at most 2 short labels of 1–2 words placed on the metaphor; (b) code, config or a serial number engraved into the anchor's own surface as **texture**, at most 3 short lines — it is there so the material reads as software at a glance, never to be read. Because it is the whole domain signal, it has to survive a small canvas: put it on a large, near, lit surface, cut big and shallow into the material. On a distant, dim or half-lit object it dissolves into noise and the image loses the only thing that made it about software. Anything beyond that is a wall of tiny labels: no paragraphs, no fake dashboards, no fake UI screenshots.
