...
read off a live system, pinned source, or upstream docs on 2026-09-13 — nothing
...
|---|---------|---------|--------|-------------|
| 1 | GoCardless API shape | `POST /api/v2/token/new/` (secret_id/key -> access **86400 s** + refresh **2592000 s**), `POST /token/refresh/`; agreement -> requisition (browser consent) -> `GET /accounts/{id}/transactions/?date_from&date_to`, Berlin Group PSD2 JSON (`booked`/`pending`) | docs.gocardless.com bank-account-data endpoints + quickstart | Token minted per run via `/token/new`; only `booked` ingested, `pending` skipped (identity unstable, breaks idempotency) |
| 2 | History depth | Default agreement = 90 days; a custom end-user agreement sets `max_historical_days` up to each institution's `transaction_total_days`; platform max 24 months | quickstart + overview docs | Onboarding creates a custom agreement at the bank's max; Spanish banks commonly cap below the platform max — `[UNVERIFIED]` per bank until WF-3 reads `GET /api/v2/institutions/?country=ES` live |
| 3 | Bank rate limits | PSD2 default as low as **4 calls/day/account per endpoint**; `RateLimitError` 429 carries a reset header | overview docs + Bank Account Data Service Terms (Jan 2026) §3.13 | Daily DAG = 1 transactions call/account — well inside. On 429 the fetch fails loudly; it never retries in a loop against a legal limit |
| 4 | Consent expiry | EEA re-authentication every **180 days** (UK 90); requisition status goes `EX` | Service Terms §3.21 + statuses doc | Fetch surfaces `ACCESS_EXPIRED` as a distinct failure naming the bank; re-link runbook in §4.2 |
| 5 | Transaction id stability | `transactionId` exists in the PSD2 shape but some institutions omit it | endpoints doc | Bronze primary key = `transactionId` when present, else a deterministic hash (account, bookingDate, amount, remittance, counterparty). Which of the five target banks omit it is `[UNVERIFIED]` until WF-3 |
...
| 13 | dlt stack | dlt pinned **1.30.0**; `httpx` already a dependency | `workflows/dlt/uv.lock:391`, `workflows/dlt/pyproject.toml` | The GoCardless client is a plain httpx resource; the only new dependency is `actualpy` |
...
**Rejected at the gate:** Actual Budget's *built-in* GoCardless bank sync would
make the fetch half of this spec unnecessary. Rejected: it bypasses the
lakehouse (no bronze history, no dbt models, no semantic metrics), and it would
run a second independent fetcher against the same 4-calls/day PSD2 budget —
two consumers of one legal rate limit, neither knowing about the other. The
lake stays the single fetcher; Actual Budget receives data only through §4.6.

**Target banks (user-stated):** CaixaBank, BBVA, ING, Openbank, Santander —
all Spanish. Institution ids and per-bank `transaction_total_days` are read
live at onboarding, never hardcoded from memory.
...
sit in front of it. Bank data arrives through GoCardless Bank Account Data —
a plain REST API with a JWT token flow — so there is no n8n, no Kafka topic
and no scraping: dlt calls the API directly. What stays identical is
everything downstream: the Iceberg medallion, the dbt project layout, the
enrich pattern, the Airflow task helpers, the Superset bundle mechanism and
the semantic registry format.

### 1.2 The problem

Personal bank transactions currently exist only inside the banks' own apps.
There is no history under the operator's control (banks and PSD2 agreements
cap what an API call returns, and UIs truncate long before that), no
cross-bank view, no analytics surface, and the budgeting tool the operator
wants to use (Actual Budget) is not deployed and would, on its own, duplicate
fetching against a legally rate-limited API.

The cost ordering:

1. **No durable history.** Every day without ingestion is a day the 90-day
   default window can no longer reach. This is the only irrecoverable cost.
2. **No single fetcher.** If Actual Budget's built-in sync and a pipeline both
   pull from GoCardless, they compete for the same per-account daily budget
   and can lock each other out (429).
...
without the lake means Actual Budget's built-in GoCardless sync is the only
...
- **The provider adapter owns wire-format knowledge.** Berlin Group PSD2
  quirks (string amounts, per-bank missing fields) are normalised once, in
  the adapter, before bronze. No dbt model parses provider JSON.
...
| 2 | Max-history backfill on first link | The onboarding agreement carries `max_historical_days` = the institution's `transaction_total_days`, and the first ingest returns rows older than 90 days for at least one bank whose cap allows it, or the run log records the bank's actual cap |
...
| 6 | Access expiry is actionable, not silent | A run against an expired requisition fails with `ACCESS_EXPIRED` naming the bank and the re-link runbook; it never reports success with zero rows |
...
- **No bidirectional sync.** Budgets, edited categories and manual
  ...
  categorisation.
- **No pending transactions.** Only `booked` entries are ingested. Pending
  entries change identity when they book, which breaks the stable-id
  idempotency goal 1 depends on.
- **No second provider implemented.** The adapter interface ships with
  GoCardless only. A Salt Edge or Plaid adapter is built when a bank appears
  that GoCardless cannot reach — not before, because an untested second
  implementation of an interface is just a guess about the interface.
...
flowchart LR
    ...
    GC -->|booked transactions, accounts, balances| ING
    ING --> BR --> SIL
    SIL --> ENR --> MC
    SIL --> GLD
    MC --> GLD
...
Trust boundary: everything left of the cluster frame. The GoCardless payload
is untrusted input — the adapter (§4.1) is the only code that touches wire
format, and it normalises before bronze. The Actual Budget server is trusted
storage but a **single-writer** resource: only `finance.sync` writes to it,
and its built-in bank sync is never configured (§5, ordering mistake).

| Layer | Component | Owns | Must not |
|-------|-----------|------|----------|
| Provider adapter | `projects/finance/providers/gocardless.py` | Token minting, requisition reads, PSD2 normalisation, error classification (429 / `ACCESS_EXPIRED` / `AccountProcessing`) | Know anything about medallion layers; retry against a rate limit |
...
`payload_json` carrying the untouched provider object. The GoCardless
implementation owns: `/token/new` per run (gate 1: access lives 24 h, the DAG
runs daily, so no refresh token is ever persisted); the requisition ->
accounts read; `?date_from&date_to` windowing; and error classification —
429 fails the run quoting the reset header (gate 3), `AccessExpiredError` /
expired EUA fails as `ACCESS_EXPIRED` naming the institution (gate 4, goal
6), `AccountProcessing` (409) fails with "retry later" semantics.
...
three-endpoints-per-account shape and — decisively — the need to classify
PSD2 error codes into pipeline-level failures with runbook text make a plain
httpx client (already a dependency, gate 13) the smaller total surface. The
interface, not dlt's source machinery, is what makes a second provider
possible later.

Which of the five banks omit `transactionId` is unknown until WF-3 (gate 5);
the stable-id rule below does not depend on the answer.

**Stable id.** `stable_id = transactionId` when the provider supplies one,
else `sha256(account_id | booking_date | amount | currency | remittance |
counterparty)` truncated to 32 hex chars. Computed in the adapter, stored in
bronze, and reused as Actual Budget's `imported_id` (§4.6) — one identity
across the whole pipeline. A hash collision across *different* transactions
at the same bank on the same day for the same amount and text is accepted:
PSD2 gives nothing better, and bodega's `invoice_number` key has the same
"trust the source's identity" shape.

### 4.2 Onboarding and token model

Linking a bank is inherently manual (browser consent) and happens once per
180 days per bank (gate 4). A helper CLI — `uv run python -m
finance.onboard`, operator laptop, never in-cluster — walks it:

1. mint a JWT from `secret_id`/`secret_key`;
2. `GET /api/v2/institutions/?country=ES` and print each target bank's
   `transaction_total_days` (closes gate 2's `[UNVERIFIED]` per bank);
3. create an end-user agreement with `max_historical_days` = that cap,
   `access_valid_for_days` = 180, scope `details,balances,transactions`;
4. create a requisition, print the consent link;
5. poll until status `LN`, then print the account ids with masked IBANs.

The operator pastes the result into the `finance-gocardless` k8s secret
(keys: `secret_id`, `secret_key`, `requisitions.json` — institution id ->
requisition id + account id list). No token, requisition or account id is
ever committed to git. Re-authentication is the same CLI re-run for the
expired bank; `ACCESS_EXPIRED` failures (§4.1) point at this runbook, which
lives in `workflows/dlt/README.md`.

*Alternative rejected:* persisting the refresh token (30-day TTL) and
refreshing. It needs either secret-write RBAC for job pods or token state in
the lake — both worse than minting a 24-h token per daily run, which is what
GoCardless's own integrations (actual-server, the OCA Odoo module) do.
...
2. `Actual(base_url, password, file=<sync id>)` — download the budget,
   ...
   (gate 7), so dedup is explicit: `imported_id = "gocardless:" + stable_id`
...
own GoCardless/SimpleFIN bank sync is never configured (§5).

### 4.7 Secrets and data privacy

Secrets (k8s, `data` namespace, wired via the existing `SecretEnvVarRef`
pattern): `finance-gocardless` (provider credentials + requisitions.json)
and `finance-actual` (server password, sync id, account map). None in git;
the onboarding CLI prints values for the operator to paste.
...
Schedule `0 6 * * *` UTC (bodega runs 08:00 — staggered; GoCardless's
per-account daily budget is per-bank so there is no contention, the stagger
is for cluster load). Params `from_date`/`to_date` default to a 14-day
lookback (§4.3). Secret wiring per task: ingest needs
ICEBERG + gocardless; enrich needs ICEBERG + LITELLM; sync needs
finance-actual. `retries: 1` like every DAG
...
- [ ] **WF-1** `workflows/dlt/projects/finance/`: package skeleton,
      `config.py` (re-exporting `dlt_runner.config`, `FINANCE_*` env
      helpers), the `BankDataProvider` protocol, the GoCardless adapter
      (token, accounts, transactions, balances, error classification incl.
      `ACCESS_EXPIRED` and 429), stable-id rule. Unit tests against recorded
      JSON fixtures (no live API in CI).
- [ ] **WF-2** `finance.onboard` CLI + the re-link runbook in
      `workflows/dlt/README.md`. *Blocked by WF-1.*
- [ ] **WF-3** Live onboarding: operator creates GoCardless secrets, links
      the first bank, records each target bank's `transaction_total_days`
      and `transactionId` presence (closes gates 2/5 `[UNVERIFIED]`),
      writes `finance-gocardless` into the cluster. *Blocked by WF-2; needs
      operator + real bank credentials.*
...
deliberately expired requisition fails as `ACCESS_EXPIRED`.
...
Budget's *built-in* GoCardless bank sync "just to see it work" before or
while WF-8 lands. Two fetchers then share one legal 4-calls/day/account
budget (gate 3): the pipeline starts failing 429s on days the app already
fetched, the failures look like a provider outage, and every transaction
arrives twice under two different identities — the app's own `imported_id`
scheme and the pipeline's — so goal 5's dedup cannot see the duplicates. The
app's bank sync stays unconfigured, permanently, and that decision is
recorded here rather than rediscovered. Runner-up: letting the server image
float on `latest` while actualpy is pinned — a server-side sync-protocol
change then corrupts budget files on commit (gate 14 exists for this).
...
| Risk | Mitigation |
|------|------------|
| A Spanish bank's PSD2 connection is flaky or drops consent early (institution-side, outside GoCardless's control) | Error classification keeps the failure named (`ACCESS_EXPIRED` vs 429 vs `ServiceError`); the 14-day overlap window means a few lost days are recovered by the next green run; bronze is append-only memory, so nothing already landed can be lost |
| A bank omits `transactionId` **and** rewrites the fields the hash is built from (remittance text is bank-formatted) — the same transaction lands twice under two stable ids | WF-3 does not just record `transactionId` presence: it fetches the same window twice a day apart and diffs, choosing hash inputs from fields observed to be stable for that bank before the first real ingest |
...
| A second writer touches the budget while sync runs (user in the UI is fine — CRDT handles concurrent clients — but Actual's built-in bank sync would not be) | Single-writer rule (§4.6): built-in GoCardless/SimpleFIN sync is never configured; the ordering mistake in §5 is the permanent record of why |
...
| GoCardless free-tier or pricing change (402 `CompanyRequisitionLimitReached`) | Personal scale is five requisitions, far under historical free limits; the adapter interface (§4.1) is the escape hatch — a second provider is an adapter, not a rewrite |
| The 180-day re-auth is forgotten until the DAG fails | Failing loudly *is* the design (goal 6): the run names the bank and the runbook; a silent zero-row success is the failure mode this spec forbids |
...
| Question | Settle by |
|----------|-----------|
| Per-bank `transaction_total_days` and `transactionId` presence for CaixaBank, BBVA, ING, Openbank, Santander (gate 2/5 `[UNVERIFIED]`) | WF-3: read `GET /api/v2/institutions/?country=ES` live and record the values in the onboarding runbook |
...
2. The first linked bank's history extends back to its recorded
   `transaction_total_days` cap (goal 2).
3. A deliberately expired or revoked requisition fails the ingest with
   `ACCESS_EXPIRED` naming the bank (goal 6).
