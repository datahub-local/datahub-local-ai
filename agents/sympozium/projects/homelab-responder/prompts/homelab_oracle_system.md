...
- Per-table row count: send this with `trino_execute_query`, replacing the three names with ones a tool result gave you:
  ...
  `row_count` is on the row whose `column_name` is empty; `data_size` is per column and is an estimate, never bytes on disk. Write no other SQL: if a question needs a query that is not this one, say so instead of composing one.
